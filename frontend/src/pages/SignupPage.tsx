...
import { Mail, Lock, User, ShieldCheck, ArrowRight, Radio, Info, Send } from 'lucide-react';
        ...
        <div className="glass p-8 rounded-[2rem] border border-white/10 relative overflow-hidden backdrop-blur-3xl">
            ...
            {!isEmailVerificationSent ? (
              ...
              >
                ...
                <div className="flex flex-col items-center mb-8 pt-4">
                  ...
                  <p className="text-slate-500 text-[9px] font-black uppercase tracking-[0.4em] mt-2">Initialize Profile Protocol</p>
                </div>
              ...
              >
                <div className="w-20 h-20 bg-cyan-500/10 border-2 border-cyan-500 rounded-full flex items-center justify-center mx-auto mb-6 shadow-[0_0_30px_rgba(6,182,212,0.2)]">
                    <Send className="text-cyan-400 animate-pulse" size={40} />
                </div>
                <h2 className="text-xl font-black uppercase tracking-tighter">Verification Signal Sent</h2>
                <p className="text-slate-400 text-[11px] leading-relaxed uppercase tracking-wider">
                  Encryption keys dispatched to <br/><span className="text-cyan-400 font-bold">{email}</span>. <br />
                  Authorize uplink to activate terminal.
                </p>
                <div className="pt-4 flex flex-col gap-3">
                    <button 
                        onClick={handleVerificationComplete}
                        className="w-full py-3.5 rounded-2xl font-black uppercase tracking-[0.2em] text-[10px] bg-slate-900 hover:bg-slate-800 text-white transition-all border border-white/5 active:scale-[0.98]"
                    >
                        Return to Protocol
                    </button>
                    <button className="text-[8px] font-black uppercase tracking-widest text-slate-600 hover:text-cyan-500 transition-colors">Resend Verification Signal</button>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          {!isEmailVerificationSent && (
            <div className="mt-8 text-center pt-6 border-t border-white/5">
                <span className="text-slate-600 text-[10px] uppercase font-black tracking-widest">Active Operator? </span>
                <Link to="/login" className="text-cyan-500 text-[10px] uppercase font-black tracking-widest hover:text-white transition-colors">Engage Protocol</Link>
            </div>
          )}
        </div>
